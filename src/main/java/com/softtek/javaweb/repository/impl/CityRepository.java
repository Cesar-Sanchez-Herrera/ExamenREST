...
public class CityRepository implements MyRepository<City,Long>{
	...
	public List<City> getByName(String name) {
		String sql = "SELECT * FROM city WHERE description like concat('%',:name,'%')";
		return namedParameterJdbcTemplate.query(sql,Collections.singletonMap("name", name), cityRowMapper);
	}
}
